  ...
  return (
    <div>
      <div className="flex items-end justify-between gap-4">
        <div>
          <h2 className="text-2xl md:text-3xl font-bold">Maps & Geo Features</h2>
          <p className="mt-2 text-muted-foreground">
            Click markers for details or type a location to highlight. Pure front-end simulation (no map SDK).
          </p>
        </div>
      </div>
        ...
        <Card className="p-4 md:col-span-2">
          <div className="relative aspect-[16/10] rounded-md border bg-muted/20">
                ...
                >
                  <span
                    className={`block h-4 w-4 rounded-full ring-2 ${
                      isSelected ? "bg-primary ring-primary" : "bg-foreground ring-background"
                    }`}
                  />
                </motion.button>
              )
            ...
            <div className="pointer-events-none absolute inset-0 grid place-items-center">
              <span className="text-xs text-muted-foreground">Stylized map placeholder</span>
            </div>
          </div>
        </Card>
        ...
        <Card className="p-4">
          ...
          <div className="mt-4 space-y-2">
            ...
            <div className="flex flex-wrap gap-2">
              {filtered.map((m) => (
                <Badge key={m.id} variant={active?.id === m.id ? "default" : "secondary"} onClick={() => setActive(m)}>
                  {m.name}
                </Badge>
              ))}
            </div>
